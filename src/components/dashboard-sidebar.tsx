    ...
    return (
      <div className="relative group">
        <button
          ...
          className={cn(
            "flex w-full items-center justify-center rounded-full p-3 transition-colors",
            "hover:bg-secondary",
            (isActive || hasActiveChild) && "bg-dashboard-purple/20 text-dashboard-purple"
          )}
        >
          <item.icon className="h-5 w-5" />
          {item.badge && (
            <span className="absolute right-1 top-1 h-2 w-2 rounded-full bg-dashboard-green" />
          )}
        </button>
        {isOpen && !isMobile && (
          <div
            className={cn(
              "absolute left-full ml-2 rounded-lg border bg-card p-2 shadow-lg",
              "min-w-[200px] z-50"
            )}
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        "hover:bg-secondary",
        isActive && "bg-dashboard-purple/20 text-dashboard-purple"
      )}
    >
      <item.icon className="h-5 w-5" />
      {item.badge && (
