...
import { Info, Save, Plus, ChevronDown, ChevronRight, Wallet } from "lucide-react"
        ...
        <div className={cn(glassCard, "bg-card dark:bg-gradient-to-r dark:from-[#1a1f35]/80 dark:to-[#101426]/80")}>
          ...
          <div className="w-full md:w-1/2">
            <div className="space-y-2">
              <label className="text-xs font-medium uppercase tracking-wider text-foreground/60">Reservation Type</label>
              <Select
        ...
        <div className="grid gap-6 grid-cols-1">
          ...
          <div className={cn(glassCard, "overflow-hidden")}>
             <div className="mb-6 flex items-center justify-between">
              <div>
                <p className="text-sm uppercase tracking-[0.3em] text-foreground/60">Cost Breakdown</p>
                <h3 className="text-xl font-semibold">Expense Analysis</h3>
              </div>
              <div className="rounded-full bg-secondary p-2 border border-border">
            ...
            <div className="overflow-x-auto rounded-xl border border-border bg-secondary">
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-border bg-secondary text-foreground/60">
                    <th className="px-6 py-4 font-medium uppercase tracking-wider text-xs w-[250px]">
                      <Button variant="ghost" size="icon" className="h-6 w-6 text-foreground/60 hover:text-foreground hover:bg-accent" type="button">
                ...
                <tbody className="divide-y divide-white/5 text-foreground">
                  {data.map((row) => (
                    <React.Fragment key={row.id}>
                      <tr className="bg-secondary/30 hover:bg-secondary transition-colors">
                          ...
                          <div className="flex items-center gap-2">
                            {row.children && (
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-6 w-6 text-foreground/60 hover:text-foreground hover:bg-accent"
                        ...
                        <td className="px-4 py-3">
                          <div className="relative">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-foreground/40">
                              $
                            </span>
                            <Input
                              type="number"
                              ...
                              className="pl-7 h-9 bg-muted/20 border-border text-foreground focus-visible:ring-ring text-right"
                              readOnly
                            />
                          </div>
                        </td>
                      ...
                      {row.isExpanded &&
                        row.children?.map((child) => (
                          <tr key={child.id} className="bg-muted/20 hover:bg-muted/30 transition-colors border-b border-border">
                            <td className="px-6 py-3 pl-14">
                              <span className="text-sm text-foreground/70">{child.label}</span>
                            </td>
                            <td className="px-4 py-3">
                              <div className="relative">
                                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-foreground/40">
                                ...
                                <Input
                                  ...
                                  className="pl-7 h-8 bg-transparent border-border text-foreground focus-visible:ring-ring focus-visible:bg-accent text-right text-sm"
                                />
                              </div>
                            </td>
                            <td className="px-4 py-3">
                            ...
                            <td className="px-4 py-3">
                              <div className="relative">
                                ...
                                <Input
                                  ...
                                  className="pl-7 h-8 bg-transparent border-transparent text-foreground/60 font-medium text-right text-sm"
                                  readOnly
                                />
                              </div>
                            </td>
